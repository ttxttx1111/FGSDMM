...
public class GSDMM
{
	...
	public HashMap<String, Integer> generateDic(int wordFreThresh) throws IOException {
		Dic dic = new Dic(dataDir + dataset);
		dic.setWordFreThresh(wordFreThresh);
		dic.generateWordToIdMap();
		return dic.wordToIdMap;
	}

	public void  getDocuments(HashMap<String, Integer> wordToIdMap) throws Exception
	{
...
//		}
	...
	public void runGSDMM(boolean useIndex,int thread_num) throws Exception
	{
		...
		model.setData(documentSet,thread_num);
		model.gibbsSampling(useIndex);
//		if(useWordProb){
...
//		model.output(documentSet, outputPath);
		this.z = model.z_i;
		this.communicationTime = model.communication_time;
